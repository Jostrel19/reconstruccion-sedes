...
| D-19 | **Backend del sistema nuevo: Google Apps Script** | **2026-09-15, precisada 2026-09-16.** Reemplaza la exploración de Logic Apps/Functions de §8.1: Apps Script + Google Sheets (como base de datos) + Drive (como repositorio de archivos), gratis, sin licencia ni registro en Entra ID. Mismo patrón que `circular122` (otro sistema de la SED). **Precisión:** el dominio `@sedcaldas.edu.co` es Microsoft 365, no Google Workspace — no existe cuenta de Google con ese correo. `circular122` está desplegado desde una cuenta de Gmail **personal** del funcionario que lo hizo, no desde el dominio institucional; este proyecto sigue el mismo patrón, con el Gmail personal del practicante. Consecuencia aceptada: el script y las hojas quedan de propiedad de esa cuenta personal, no de una cuenta de la SED — mismo riesgo de continuidad que ya asume `circular122`, no uno nuevo que este proyecto introduzca. `api.js` no cambia de forma — su modo `'http'` ya existía; solo cambia a qué URL apunta |
...
| D-31 | **La priorización se deriva del tipo de afectación, no de una lista aparte** — *revisada por D-44: el tipo 1-2 se muestra, pero los lotes los crea el Administrador* | Priorizada = `A2. TIPO DE AFECTACIÓN` tipo **1** (colapso total o parcial) o **2** (riesgo inminente) — los dos que el propio censo marca `PRIORITARIO`. Da **37 sedes en 12 municipios**, $9.718.464.165 de referencia, las 37 con valor. `data/insumos/PrimerasPriorizadas.xlsx` (33 sedes) **queda retirado como fuente**: dos listas paralelas se desincronizan, y de hecho ya lo habían hecho (ver §3, hallazgo de reclasificación) |
| D-37 | **Ningún dato de `Presupuestos`/`Items` se sobrescribe, y el Sheet se protege contra edición directa** | **2026-09-17.** Preocupación real: al ser Google Sheets la base de datos (D-19), alguien con acceso de editor podría cambiar una celda **a mano, por fuera de la API**, y ese cambio no dejaría ningún rastro — ni versión anterior, ni autor, ni fecha. Se cierra en dos capas: **(1) aplicación** — el backend nunca hace `UPDATE` sobre una fila de `Presupuestos` o `Items`; cada corrección es una fila **nueva** con `version` +1, la fila anterior pasa `vigente` a `FALSE` pero **nunca se borra** — el histórico completo queda siempre disponible. **(2) hoja de cálculo** — `Datos > Hojas y rangos protegidos` restringe la edición de `Presupuestos`, `Items`, `Verificaciones`, `Usuarios` y `Hallazgos` a **solo el dueño del Sheet**; el script sigue escribiendo porque se ejecuta con los permisos del dueño, pero ningún otro humano con acceso a la hoja puede tocar una celda directamente — el único camino para cambiar un dato pasa por la API, que sí versiona. `Sedes` no se protege igual porque se regenera completa desde el catálogo en cada build, así que una edición manual ahí se pierde sola en el siguiente build, no se disfraza de dato real |
| D-38 | **Tres medidas de resiliencia operativa: bloqueo por intentos, respaldo diario, runbook de continuidad** | **2026-09-17.** Cierran huecos detectados en una revisión de qué tan sólido es el sistema para lo que se necesita. **(1)** `Auth_validarCodigo` bloquea un código tras **5 intentos fallidos** (antes no había límite: un script podía probar las 999.999 combinaciones dentro de los 10 minutos de vigencia). **(2)** `backend/Backup.gs::configurarRespaldoAutomatico` crea un disparador diario que copia el Sheet completo a una carpeta aparte de Drive, con 30 días de retención — el historial de versiones nativo de Google vive dentro del mismo archivo y no protege contra que ese archivo se pierda o quede inaccesible. **(3)** `docs/RUNBOOK_CONTINUIDAD.md` documenta qué hacer si quien mantiene la cuenta personal de Gmail (D-19 precisada) deja la práctica sin poder coordinar el traspaso: inventario de qué vive dónde, y los dos procedimientos (traspaso coordinado / recuperación de emergencia desde el respaldo). No resuelve la causa de fondo —seguir dependiendo de una cuenta personal— pero reduce el costo de que ese riesgo se materialice |
| D-39 | **El concepto de verificación cambia el `estado` de la fila vigente; no crea una versión nueva** | **2026-09-21.** Extiende el mismo patrón que D-37 ya autorizó para el campo `vigente`: `Verificaciones_emitir` hace un único `setValue` dirigido sobre la celda `estado` de la fila vigente de `Presupuestos` — nunca reescribe costo directo, ítems ni ningún otro dato de esa fila. El concepto completo (resultado, observaciones, quién, cuándo) vive íntegro en `Verificaciones` (D-21), fila que nunca se sobrescribe: la auditoría no depende de la celda que sí cambia. Se descartó minar una versión nueva de `Presupuestos` por cada concepto porque el contenido del presupuesto no cambió —solo su estado— y habría inflado el historial con versiones idénticas salvo por una palabra. Antes de escribir, `Verificaciones_emitir` exige que el `id_presupuesto` sobre el que se emite siga siendo el vigente; si el municipio radicó una versión nueva mientras el arquitecto tenía la pantalla abierta, rechaza el concepto en vez de aplicarlo sobre una versión que ya dejó de ser la actual |
| D-40 | **Cargas: quién puede volcar y qué se premarca** — *premarcado revisado por D-44: ya no se mira el lote* | **2026-09-21.** `Presupuestos_guardar` distingue `origen: MANUAL \| CARGA`: MANUAL sigue exigiendo `RESPONSABLE_SEDE` (o Administrador); CARGA además admite `VERIFICADOR` — el arquitecto sube lo que el municipio ya mandó en Excel (D-33), pero sigue sin diligenciar a mano (alcance por rol, `docs/PLAN_DESARROLLO.md` §2). En la pantalla, las filas del JSON que produce `tools/ingesta.py --json` que quedan **fuera del lote 1** (`priorizada=FALSE`) no vienen premarcadas para volcar aunque su DANE esté confirmado por archivo: el catálogo completo de 975 ya vive en el sistema, pero el ciclo activo de seguimiento sigue siendo el lote 1 (`docs/ALCANCE_Y_REQUISITOS.md` §6) — cargarlas queda disponible si el arquitecto lo decide fila por fila, no por defecto. Las filas con `dane_origen=propuesto` (el lector no encontró DANE en el archivo, lo dedujo por nombre) tampoco vienen premarcadas nunca, sin importar el lote: marcar la casilla de esa fila es en sí misma la confirmación humana que exige el propio lector (`tools/ingesta_esquema.py::RegistroIngesta`) |
| D-41 | **Un volcado sin desglose por capítulo se guarda como un ítem único, no como un total suelto** | **2026-09-21.** Verificado contra los 4 lectores reales (`tools/ingesta_*.py`), corriéndolos de nuevo: ninguno —tampoco Belalcázar— exporta hoy ítems por capítulo ni A/U/IVA/plazo; `RegistroIngesta` solo trae `costo_directo`. Cargas vuelca ese valor como **un ítem único** (`capitulo` vacío, unidad `gl`, cantidad 1, valor unitario = costo directo) en vez de escribir un total suelto en `Presupuestos` sin ninguna fila en `Items`. Consecuencias: **(1)** `Presupuestos_guardar` no necesita ninguna rama especial para Cargas — el costo directo se recalcula igual que siempre, sumando cantidad × valor unitario; **(2)** el ítem aparece en «Presupuesto a detalle» de la Ficha en vez de quedar invisible; **(3)** el cruce daño↔presupuesto (D-34) lo ve como «sin desglose» y no marca ningún capítulo como cubierto — nunca se infiere a cuál pertenece un texto libre (D-36). `pct_admin`/`pct_utilidad` quedan en 0 (el archivo de origen no los separa de forma confiable, hallazgo H-14) y `descripcion_afectacion` deja una nota explícita de que faltan por confirmar, para que ese 0 % no se lea como una declaración real del municipio |
| D-42 | **Endurecimiento antes de la prueba real** | **2026-09-23.** Cuatro medidas: **(1)** `LockService` (candado de 10 s) en `Presupuestos_guardar` y `Verificaciones_emitir`, para que dos escrituras simultáneas sobre la misma sede no dejen dos filas `vigente` ni un concepto sobre una versión ya reemplazada; **(2)** `Fotos_subir` revisa los primeros bytes reales del archivo (JPEG `FF D8 FF`, PNG `89 50 4E 47`) en vez de confiar en el `tipo_mime` que manda el navegador; **(3)** `listarFotos` comparte cada foto como «cualquiera con el enlace, solo lectura» y devuelve `url` y `miniatura` para mostrarlas en la Ficha — trade-off aceptado: los usuarios no tienen identidad de Google (D-20), el enlace no es listable ni adivinable, y la foto es evidencia de daño en infraestructura pública, no dato personal; **(4)** Hallazgos se **detectan** solos al radicar o cargar (`Hallazgos.gs::_detectarHallazgosAutomaticos`: AIU sobre umbral, costo directo en $0, «sin afectación» sobre una sede tipo 1-2, DANE confirmado a mano en Cargas), pero **nunca se resuelven solos**: resolver exige que una persona confirme el dato y lo marque |
| D-43 | **El sistema solo lleva lo que se registra en él: sale `valor_referencia` de todas las pantallas** | **2026-09-24, decisión del usuario. Revisa D-27 y D-28; deja sin objeto el mecanismo de D-6.** La cifra estimada del censo (`PRESUPUESTO APROX INVERSION` de `dimDaños`) se estaba leyendo como si fueran registros ya radicados, y confundía. El sistema pasa a ser solo el control y seguimiento de los presupuestos que se registran en él. **Qué cambia:** `Sedes_listar` y `Verificaciones_bandeja` dejan de enviar `valor_referencia` a cualquier rol; Tablero, Sedes, Municipio, Ficha y Verificación muestran estado, conteos, valor radicado y bitácora calculados de `Presupuestos`/`Verificaciones` reales (`Sedes_listar` adjunta a cada sede el resumen de su versión vigente y devuelve la actividad reciente). **Qué no cambia:** la columna `valor_referencia` sigue en la pestaña `Sedes` (no se borra información del censo, y se regenera con el catálogo); el tipo de afectación y los capítulos con daño del censo **sí** se siguen mostrando, porque definen el lote (D-31) y el cruce daño↔presupuesto (D-34). **«En seguimiento»** = sedes del lote 1 más cualquier sede con presupuesto registrado, para que nada de lo que se registra quede fuera de la vista (*redefinido por D-44*) |
...
**`Lotes`** y **`LotesSedes`** (D-44, 2026-09-24 — lotes creados por el Administrador; `backend/Lotes.gs`):
